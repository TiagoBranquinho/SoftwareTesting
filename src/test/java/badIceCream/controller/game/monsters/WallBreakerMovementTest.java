...
public class WallBreakerMovementTest {
    ...
    void setUp() {
        ...
        when(mockArena.isIceWall(any())).thenReturn(false);

        when(mockArena.getIceCream()).thenReturn(mockIceCream);
        when(mockArena.isEmptyNoStoneWall(any())).thenReturn(false);
    }
    ...
    void testStep_MovesMonsterToValidPosition() throws IOException {
        ...
        when(mockArena.isEmptyNoStoneWall(newPosition)).thenReturn(true);

        wallBreakerMovement.step(mockMonster, mockArena, 200, 0);

        //verify arena.iceWallDestroyed isnt called
        verify(mockArena, never()).iceWallDestroyed(newPosition);

        verify(mockMonster).setPosition(newPosition); // Verify the monster moves
    }

    @Test
    void testDestroyIceWall() throws IOException {
        Position monsterPosition = new Position(5, 5);
        Position newPosition = new Position(5, 6);
        ...
        when(mockArena.isIceWall(any())).thenReturn(true);

        wallBreakerMovement.step(mockMonster, mockArena, 200, 0);

        verify(mockArena).iceWallDestroyed(newPosition); // Verify the ice wall is destroyed
    ...
    void testStep_NoMoveDueToDelay() throws IOException {
        ...
        wallBreakerMovement.step(mockMonster, mockArena, 199, 0); // Not enough time has passed
    ...
    void testMoveMonster_CollidesWithIceCream() {
        ...
        wallBreakerMovement.moveMonster(mockMonster, monsterPosition, mockArena);
    ...
    void testLastMoveDown() throws IOException {
        ...
        when(mockArena.isEmptyNoStoneWall(endPosition)).thenReturn(true);

        wallBreakerMovement.step(mockMonster, mockArena, 225, 0);
